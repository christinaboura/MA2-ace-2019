...
Soit $$\newcommand{\F}{\mathbb{F}} P(x) ∈ \F_{13}[x]$$ le polynome
...
1. Calculer un générateur $$g$$ pour l'idéal $$I_P ∩ \F_{13}[x]$$.

2. Quelle est la dimension de $$I_P$$ ? Quelle est la dimension de
   $$\F_{13}[x,\bar{y}]/I_P$$ en tant qu'espace vectoriel ?

3. Vérifier que $$g$$ se scinde dans $$\F_{13^4}$$, vérifier que toute
   racine de $$P$$ dans $$\F_{13^4}$$ est aussi racine de $$g$$.

4. Prouver que pour tout polynôme $$P$$ la variété $$V(I_P ∩
   \F_{13}[x])$$ est de dimension zéro (dans $$\F_{13}[x]$$) et
   coincide avec les racines de $$P$$ dans $$\F_{13^4}$$.

5. Calculer des générateurs des idéaux $$I_P∩\F_{13}[y_1]$$, $$I_P∩\F_{13}[y_2]$$
   et $$I_P∩\F_{13}[y_3]$$ (pour le $P$ donné plus haut).
...
1. Soient $$f=\sum_i f_ix^i$$, $$g=\sum_i g_ix^i$$ et $$h=\sum_i
   h_ix^i$$ trois polynômes à coefficients dans $$\F_p$$. Prouver que
   $$h=fg$$ si et seulement si
   
   $$\left(\sum_i f_ix^{p^i}\right)∘\left(\sum_i g_ix^{p^i}\right) = \sum_i h_ix^{p^i}.$$

2. Soit $$ζ∈\F_p$$ un élément d'ordre multiplicatif égal à $$n$$.
   Prouver que $$(x^p - ζ^nx)∘\cdots∘(x^p - ζ^2x)∘(x^p - ζx) = x^{p^n} - x$$.
...
4. Soient $$α_1, \dots, α_d$$ des éléments de $$\F_{p^n}$$. Soit $$P$$
   le polynôme
...
5. Soit $$P$$ un polynôme à coefficients dans $$\F_{p^n}$$, et soit $$I_P$$ l'idéal
   
   $$I_P = 〈 P(y_0) 〉 + \left\langle (y_{i+1} - y_i^p + ζ^iy_i)_{0≤i<n-1} \right\rangle
   + 〈 y_{n-1}^p - y_{n-1} 〉$$
   
   Prouver que $$V(I_P ∩ \F_p[y_i]) = \{L_i(α) \mid α∈\F_{p^n}, P(α)=0\}$$
   pour tout $$i<n$$.

6. Prouver que $$V(I_P ∩ \F_p[y_{n-1}]) ⊂ \F_p$$.


## Un algorithme de recherche de racines

Réaliser en Sage un algorithme de recherche de racines dans
$$\F_{p^n}$$. L'algorithme procède en calculant $$V(I_P∩\F_p[y_{n-1}])$$
à l'aide d'une suite de $$n$$ résultants puis en résolvant $$n$$
systèmes linéaires pour calculer successivement,
$$V(I_P∩\F_p[y_{n-2}])$$, ..., $$V(I_P∩\F_p[y_{n-1}])$$.
...
>    
>    - Calculer $$P_{i+1} = \mathrm{Res}_{y_i}(P_i(y_i),\; y_{i+1} - y_i^p + ζ^iy_i)$$
> 
> 2. Calculer $$V_{n-1} = V(I_P∩\F_p[y_{n-1}])$$ par recherche exaustive dans $$\F_p$$.
> 
